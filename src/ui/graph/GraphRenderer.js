...
class GraphRenderer extends PureComponent {
  ...
  handleData(data)  {
    var id = data.stepid;
    var cell = this.dynagramElements[id]
  ...
  componentDidMount() {
    ...
    window.mxUtils.getAll(['', '/lib/mxGraph/styles/default.xml'], function(xhr) {
      ...
      if (_this.props.activegraph.graph != null) {
        ...
        store.dispatch({type: 'SET_GRAPH_CELLS', payload: {graphcells:_this.dynagramElements}});
      }
  ...
  loadGraphXML(xml) {
    ...
    while (elt != null){
      ...
      if (elt.nodeName == 'object')  {
        dynId = elt.getAttribute('dynagram-id');
        cell.setValue(elt.getAttribute('label'));
      }

      if (dynId != null)  {
        this.dynagramElements[dynId] = cell;
      }
      cells.push(cell);
      this.graph.refresh();
      elt = elt.nextSibling;
